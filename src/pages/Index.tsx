...
const SERVICES = [
  {
    ...
    desc: "Монтаж IP и аналоговых камер любого класса. Разрешение до 4K, ночное видение, детекция движения.",
  },
  {
    icon: "Shield",
    title: "Охранная сигнализация",
    desc: "Комплексные системы защиты периметра с мгновенным оповещением на пульт охраны.",
  },
  {
    icon: "Wifi",
    title: "Удалённый мониторинг",
    desc: "Онлайн-просмотр с любого устройства 24/7. Облачное хранение записей до 90 дней.",
  },
  {
    icon: "Lock",
    title: "Контроль доступа",
    desc: "Пропускные системы, биометрия, RFID-карты для офисов, складов и производств.",
  },
  {
    icon: "Zap",
    title: "Монтаж «под ключ»",
    desc: "Проектирование, прокладка кабелей, настройка и сдача объекта за 1–5 дней.",
  },
  {
    icon: "Headphones",
    title: "Техобслуживание",
    desc: "Ежеквартальные регламентные работы, оперативный выезд специалиста по заявке.",
  },
];

const PORTFOLIO = [
  {
    title: "ТЦ «Центральный»",
    desc: "64 камеры, 3 сервера записи",
    tag: "Торговый центр",
    color: "from-cyan-900/40 to-blue-900/40",
  },
  {
    title: "Завод «МеталлСтрой»",
    desc: "128 камер, СКУД на 4 входа",
    tag: "Производство",
    color: "from-indigo-900/40 to-purple-900/40",
  },
  {
    title: "ЖК «Горизонт»",
    desc: "32 камеры, домофония, СКУД",
    tag: "Жилой комплекс",
    color: "from-teal-900/40 to-cyan-900/40",
  },
  {
    title: "Банк «Регион»",
    desc: "48 камер 4K, 2 поста охраны",
    tag: "Финансы",
    color: "from-blue-900/40 to-indigo-900/40",
  },
  {
    title: "Склад логистики",
    desc: "80 камер, периметральная охрана",
    tag: "Логистика",
    color: "from-cyan-900/40 to-teal-900/40",
  },
  {
    title: "Офисный центр",
    desc: "24 камеры, СКУД, видеоаналитика",
    tag: "Офис",
    color: "from-slate-900/60 to-blue-900/40",
  },
];
...
function Calculator() {
  ...
  const [camType, setCamType] = useState("ip2");
  const [objType, setObjType] = useState("office");
  const [archive, setArchive] = useState("7");
  const [includeNvr, setIncludeNvr] = useState(true);
  const [includeInstall, setIncludeInstall] = useState(true);

  const camPrice = CAMERA_TYPES.find((c) => c.id === camType)!.price;
  const objMult = OBJECT_TYPES.find((o) => o.id === objType)!.mult;
  const archivePrice = ARCHIVE_OPTIONS.find((a) => a.id === archive)!.price;
  const nvrPrice = includeNvr ? Math.ceil(cameras / 8) * 18000 : 0;
          ...
          <div className="space-y-2 text-sm">
            ...
            {includeNvr && (
              <div className="flex justify-between text-gray-400">
                ...
                <span>{nvrPrice.toLocaleString("ru")} ₽</span>
              </div>
            )}
            {archive !== "7" && (
              <div className="flex justify-between text-gray-400">
                <span>Доп. архив</span>
                <span>{archivePrice.toLocaleString("ru")} ₽</span>
              </div>
            )}
            {includeInstall && (
              <div className="flex justify-between text-gray-400">
                <span>Монтаж</span>
                <span>{installPrice.toLocaleString("ru")} ₽</span>
              </div>
            )}
          </div>
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 pt-32 pb-20">
          <div className="max-w-3xl">
            ...
            <div className="flex flex-col sm:flex-row gap-4 fade-in-up delay-300">
              ...
              <a
                href="#services"
                className="px-8 py-4 rounded-xl text-sm font-semibold tracking-wide border border-white/15 hover:border-cyan-400/40 hover:bg-white/5 transition-all duration-300 inline-flex items-center gap-2 justify-center"
              >
                Наши услуги
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  { icon: "Users", text: "45 специалистов" },
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 max-w-5xl mx-auto">
            <div className="neon-border rounded-2xl p-8 bg-[#060b12]">
              <div className="space-y-5">
                ...
                <button className="neon-btn w-full py-4 rounded-xl font-semibold tracking-wide">
                  Отправить заявку
                </button>
              </div>
            </div>
